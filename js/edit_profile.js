jQuery(document).ready(function(){
	...
	jQuery.post(base_url+"user/get_expr_ajax",{}, function(data){
			if(data.status == 'ok'){
				if(data.expr_count == 0){
					var show_btn = document.getElementById('expr_edit_btn');
					show_btn.style.visibility = 'hidden';
					jQuery('#no_expr').html('There is no Experiences');					
					}
				}
			},"json");
			...
			if(data.status == 'ok'){
				if(data.skill_count == 0){
					var show_btn = document.getElementById('skill_edit_btn');
					show_btn.style.visibility = 'hidden';
					jQuery('#no_skill').html('There is no Skills');					
					}
				}
			},"json");
			...
			if(data.status == 'ok'){
				if(data.edu_count == 0){
					var show_btn = document.getElementById('edu_edit_btn');
					show_btn.style.visibility = 'hidden';
					jQuery('#no_edu').html('There is no Studies');					
					}
				}
			},"json");
...
jQuery.post(base_url+"user/update_accomplishments" ,{ accomplishments: accomplishments }, function(data){
	if(data.status == 'ok'){
							...
							jQuery('#edit_area_accomplishments').hide();
							 ...
							 jQuery('#no_accomplishments').html('');
	}
		},"json");
}else{
	jAlert("Empty Field...");
...
jQuery('#done_add_edu').click(function(){
	jQuery.post(base_url+"user/get_edu_ajax",{}, function(data){
			if(data.status == 'ok'){
				var counter = ++data.edu_count;
...
jQuery.post(base_url+"user/new_edu" ,{ school : school , grad_year : grad_year , country : country , field_study : field_study , degree : degree , details : details }, function(data){
						var t = '';	
							t +='<br />'; 
							 t +='<div class="edu_one" id="'+data.id+'"  style="font-size:14px">';
                    t +='<div class="delete_edu" id="'+data.id+'" ><img src="'+base_url+'images/Delete.png" width="12" height="12" title="Delete"/></div>';
                        t +='<p class="text" id="edu">';
                        t +='<div style="margin-left:30px">';
                          t +='<strong style="font-size:16px ; color:#111" class="school_field'+counter+'">'+data.school+'</strong><br />';
                          t +='<span style="color:#a1a1a1" ><span class="study_field_field'+counter+'">'+data.field_study+'</span> - <span class="degree_field'+counter+'">'+data.degree+'</span> - <span class="country_field'+counter+'">'+data.country+'</span> - <span class="grad_year_field'+counter+'">'+data.grad_year+'</span></span><br />';
                          t +='<span class="edu_datails_field'+counter+'">'+details+'</span>';
                        t +='</div>';
                        t +='</p>';
                        t +='</div>';
						
							jQuery('.edu_text').append( t ).hide().fadeIn(1000);
							jQuery('#new_edu').hide();
							jQuery('.new_school').val('');
							jQuery('.new_grad_year').val('');
							jQuery('.new_country').val('');
...
function add_delete_handlers()
{
	...
	jQuery( btn ).hover( function(){
		...
		jQuery('#' + btn.id).animate({
          backgroundColor: "#FEF4F4",
		  borderColor: "1px dashed #FC9696"
        }, 500 );
...
jQuery( '.delete_skill').each( function(){
	...
	jQuery( btn_skill ).hover( function(){
		
		jQuery('#' + btn_skill.id).animate({
          backgroundColor: "#FEF4F4",
		  borderColor: "1px dashed #FC9696"
        }, 500 );
	...
	jQuery( '.delete_edu').each( function(){
	
	var btn_edu = this ;
	
	jQuery( btn_edu ).hover( function(){
		
		jQuery('#' + btn_edu.id).animate({
          backgroundColor: "#FEF4F4",
		  borderColor: "1px dashed #FC9696"
        }, 500 );
	...
	jQuery( btn_edu ).click( function(){
		jConfirm('Do You Really Want to Delete it?', 'Delete', function(r) {
                    if( r ){
		edu_delete( btn_edu.id );
		...
		jQuery.post(base_url+"user/delete_expr" ,{ id : expr_id }, function(data){
			...
			if(data.status == 'ok'){
				if(data.expr_count == 0){
					var show_btn = document.getElementById('expr_edit_btn');
					show_btn.style.visibility = 'hidden';				
					jQuery('#no_expr').html('There is no experience');
					}
				}
			},"json");
...
function skill_delete( skill_id )
	...
	function edu_delete( edu_id )
	{
		jQuery.post(base_url+"user/delete_edu" ,{ id : edu_id }, function(data){
			if(data.status == 'ok'){
							jQuery( '#' + edu_id ).hide(1500);
							jQuery.post(base_url+"user/get_edu_ajax",{}, function(data){
			if(data.status == 'ok'){
				if(data.edu_count == 0){
					var show_btn = document.getElementById('edu_edit_btn');
